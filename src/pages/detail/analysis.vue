  ...
  <div class="sales-board">
    ...
    <div class="sales-board-form">
      ...
      <div class="sales-board-line">
        ...
        <div class="sales-board-line-right">
          <VMulChooser :selections="versionList" @on-change="onParamChange('versions', $event)"></VMulChooser>
        </div>
      </div>
      <div class="sales-board-line">
    ...
    <MyDialog :isShow="isShowPayDialog" @on-close="hidePayDialog">
      ...
      <VBankChooser @on-change="onChangeBanks"></VBankChooser>
      <div class="button buy-dialog-btn" @click="confirmBuy">确认购买</div>
    </MyDialog>
    <CheckOrder :isShowCheckDialog="isShowCheckOrder" @on-close-check-dialog="hideCheckOrder" :orderId="orderId"></CheckOrder>
    <MyDialog :isShow="isShowErrDialog" @on-close="hideErrDialog">支付失败！</MyDialog>
  </div>
</template>
  ...
  methods: {
    ...
    getPrice () {
      ...
      this.$http.post('/api/getPrice', reqParams)
        .then((res) => {
          if (res.status === 200) {
            this.price = res.body.data.amount * this.buyNum
          }
        }, (err) => {
          console.log(err)
        })
